...
export function WeeklyRevenueEntry() {
  ...
  const handleSubmit = async () => {
    ...
    if (!selectedEmployee || !selectedWeek || !revenueAmount || amount <= 0) {
      toast({
        variant: "destructive",
        title: "Σφάλμα",
    ...
    try {
      ...
      // Update employee's monthly actual revenue
      const currentMonthEntries = weeklyEntries.filter(entry => 
        entry.employee_id === selectedEmployee &&
        new Date(entry.week_start_date).getMonth() === new Date().getMonth()
      );

      // Calculate new monthly total
      const otherWeeksTotal = currentMonthEntries
        .filter(entry => entry.week_start_date !== selectedWeek)
        .reduce((sum, entry) => sum + entry.revenue_amount, 0);
      
      const newMonthlyTotal = otherWeeksTotal + revenueInCents;

      // Update employee's monthly actual revenue
      const { error: updateError } = await supabase
        .from('employees')
        .update({ monthly_revenue_actual: newMonthlyTotal })
  ...
  const getExistingEntry = (employeeId: string, weekStart: string) => {
  ...
  const existingEntry = selectedEmployee && selectedWeek ? 
    getExistingEntry(selectedEmployee, selectedWeek) : null;
    ...
    <div className="min-h-screen bg-background pb-20 px-4 pt-6">
      <div className="max-w-md mx-auto space-y-6">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">Εβδομαδιαία καταχώρηση τζίρου ανά εργαζόμενο</p>
        </div>
        ...
        <Card className="bg-card border shadow-sm">
          ...
          <CardContent className="space-y-4">
            ...
            {existingEntry && (
              <div className="p-3 bg-muted/50 rounded-lg border border-urban-border">
                <p className="text-sm text-muted-foreground">
                  Υπάρχει καταχώρηση: €{(existingEntry.revenue_amount / 100).toFixed(2)}
                </p>
            ...
            <Button 
              ...
              disabled={!selectedEmployee || !selectedWeek || !revenueAmount || parseFloat(revenueAmount) <= 0}
              className="w-full bg-gradient-gold hover:bg-gradient-gold/90 text-primary-foreground font-medium"
            >
              <Save className="h-4 w-4 mr-2" />
              {existingEntry ? "Ενημέρωση" : "Καταχώρηση"} Τζίρου
            </Button>
          </CardContent>
        </Card>
